        ...
        foreach ($menu as $item) {
            ...
            if (!$level) {
            	...
            	if ($item['children']) {
            		...
            		$iconmenu = $item['children'];
            	
            		foreach ($iconmenu as $iconitem) {
            		
            			$inurl = $iconitem['url'];
				
						if (stristr(($inurl),'dashboard')) $iconin = "dashboard";
						if (stristr(($inurl),'sales')) $iconin = "shopping-cart";
						if (stristr(($inurl),'catalog')) $iconin = "edit";
						if (stristr(($inurl),'system')) $iconin = "cog";
						if (stristr(($inurl),'report')) $iconin = "bar-chart";
						if (stristr(($inurl),'customer')) $iconin = "user";
						if (stristr(($inurl),'promo')) $iconin = "money";
						if (stristr(($inurl),'newsletter')) $iconin = "rss";
            	...
            	$html .= '<li id><li id=parent><a class="dropdown-toggle" href="#"><i class="icon-'.$iconin.'"></i> <span>'.$this->escapeHtml($item['label']).'<i class="icon-caret-down"></i></span></a>'. PHP_EOL;
            	$html .= '<ul class="submenu" style="display: none;">'. PHP_EOL;
            	if (empty($item['children'])) $html .= '<li id=child><a href="'.$item['url'].'">'.$this->escapeHtml($item['label']).'</a></li>'. PHP_EOL;
            } else { 
            	if ($item['url'] != "#") $html .= '<li id=child><a href="'.$item['url'].'">'.$this->escapeHtml($item['label']).'</a></li>'. PHP_EOL;
            };
            ...
            if (!$level) $html .= '</li></li>' . PHP_EOL;
            if (!$level) $html .= '</ul>' . PHP_EOL;
        }
